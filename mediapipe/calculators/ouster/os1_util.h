...
std::function<void(const uint8_t*, iterator_type it, tf::TTypes<float, 3>::Tensor&, bool&)> batch_to_iter(
    ...
    return [=](const uint8_t* packet_buf, iterator_type it, tf::TTypes<float, 3>::Tensor& map_tensor, bool& complete) mutable {
        // std::cout << "type(tf_tensor)" << typeid(tf_tensor.tensor<float, 2>()).name() << std::endl;
    
        // Eigen::TensorMap<Eigen::Tensor<float, 2, Eigen::RowMajor, Eigen::DenseIndex>> eigen_tensor(tf_tensor.flat<float>().data(), 65536, 3);
        for (int icol = 0; icol < OS1::columns_per_buffer; icol++) {
            ...
            if (f_id + 1 == cur_f_id || m_id >= W){
                std::cout << "Out of bound " << m_id << std::endl;
                continue;
            };
            ...
            for (uint8_t ipx = 0; ipx < H; ipx++) {
                ...
                int ind = 3 * (idx + ipx);
                map_tensor(0,idx + ipx, 0) = r * 0.001f * xyz_lut[ind + 0];
                map_tensor(0,idx + ipx, 1) = r * 0.001f * xyz_lut[ind + 1];
                map_tensor(0,idx + ipx, 2) = r * 0.001f * xyz_lut[ind + 2];
            ...
            if (m_id == 1023){
                complete = true;
                std::cout << "Current Measurement ID " << m_id << std::endl;
            }else{
                complete = false;
            }
        }
